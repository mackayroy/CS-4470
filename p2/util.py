...
This module provides various data structures and helper functions used throughout
the Pacman AI projects, including priority queues, counters, grids and more.
...
from typing import List, Dict, Any, Optional, Tuple, TypeVar, Generic, Callable, Union, Iterator
import sys
import inspect
import heapq
import random
...
T = TypeVar('T')  # Generic type for items in containers
...
class Stack(Generic[T]):
    ...
    Attributes:
        list: Internal list storing the stack items
    """
...
class Queue(Generic[T]):
    ...
    Attributes:
        list: Internal list storing the queue items
    """
...
class PriorityQueue(Generic[T]):
    ...
    Each inserted item has a priority associated with it and the client is usually interested
    in quick retrieval of the lowest-priority item in the queue. This data structure allows 
    O(1) access to the lowest-priority item.
    
    Attributes:
        heap: List of (priority, count, item) tuples representing the heap
        count: Counter used to break ties between items with equal priority
    """
    ...
    def update(self, item: T, priority: float) -> None:
        """Update the priority of an item or add it if not present.
...
class PriorityQueueWithFunction(PriorityQueue[T]):
    """Priority queue where priority is determined by a function.
    ...
    def __init__(self, priorityFunction: Callable[[T], float]) -> None:
        """Initialize the priority queue with a priority function.
    ...
    def push(self, item: T) -> None:
        """Add an item to the queue with priority determined by priorityFunction.
...
class Counter(Dict[Any, float]):
    """A counter keeps track of counts for a set of keys.

    The counter class is an extension of the standard python dictionary type.
    It is specialized to have number values (integers or floats), and includes a handful of
    additional functions to ease the task of counting data. In particular, all keys are
    defaulted to have value 0.
    """

    def __getitem__(self, idx: Any) -> float:
        """Get an item's count, defaulting to 0 if not present."""
        self.setdefault(idx, 0)
        return dict.__getitem__(self, idx)

    def incrementAll(self, keys: List[Any], count: float) -> None:
    ...
    def sortedKeys(self) -> List[Any]:
        ...
        Returns:
            List of keys sorted by values in descending order
        """
    ...
    def normalize(self) -> None:
        """Normalize the counter such that the total count of all keys sums to 1.
    ...
    def divideAll(self, divisor: float) -> None:
        ...
        Args:
            divisor: Number to divide all values by
        """
        for key in self:
            self[key] /= divisor
    ...
    def __add__(self, y: 'Counter') -> 'Counter':
        ...
        Returns:
            New counter with counts summed
        """
        addend = Counter()
        for key in self:
            if key in y:
    ...
    def __sub__(self, y: 'Counter') -> 'Counter':
        """Subtract counter y from counter x.
        
        Args:
            y: Counter to subtract
            
        Returns:
            New counter with counts subtracted
        """
        addend = Counter()
        for key in self:
            if key in y:
...
def raiseNotDefined() -> None:
    """Raise an error for unimplemented methods with helpful debug info."""
...
def normalize(vectorOrCounter: Union[List[float], Counter]) -> Union[List[float], Counter]:
    ...
    Args:
        vectorOrCounter: List of numbers or Counter to normalize
...
def nSample(distribution: List[float], values: List[T], n: int) -> List[T]:
...
def sample(distribution: Union[List[float], Counter], values: Optional[List[T]] = None) -> T:
    """Sample once from a distribution.
    
    Args:
        distribution: List of probabilities or Counter
        values: Optional list of values to sample from
...
def sampleFromCounter(ctr: Counter) -> Any:
    """Sample from a Counter object.
    
    Args:
        ctr: Counter object to sample from
        
    Returns:
        One sample from the counter
    """
    items = sorted(ctr.items())
    return sample([v for k, v in items], [k for k, v in items])

def getProbability(value: Any, distribution: List[float], values: List[Any]) -> float:
...
def chooseFromDistribution(distribution: Union[Dict[T, float], Counter, List[Tuple[float, T]]]) -> T:
    """Choose an item from a distribution.
    
    Args:
        distribution: Either a Counter, dict, or list of (prob, item) pairs
...
def arrayInvert(array: List[List[T]]) -> List[List[T]]:
    """Invert a matrix stored as a list of lists.
    
    Args:
        array: Matrix to invert
...
def matrixAsList(matrix: List[List[Any]], value: Any = True) -> List[Tuple[int, int]]:
    """Turn a matrix into a list of coordinates matching the specified value.
...
def lookup(name: str, namespace: Dict[str, Any]) -> Any:
    ...
    Args:
        name: Name of object to look up
    ...
    Raises:
        Exception: If object not found or multiple objects found
    """
    dots = name.count('.')
    if dots > 0:
...
class TimeoutFunction:
    """Function wrapper that raises an exception on timeout."""
    ...
    def __call__(self, *args: Any, **keyArgs: Any) -> Any:
        """Call the wrapped function with timeout checking."""
